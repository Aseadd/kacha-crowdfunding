...
import { useMasterCardWebhookMutation } from "src/api/data";
...
export default function DonationConfirmMasterCard() {
    const [message, setMessage] = useState<string[]>();
    const [success, setSuccess] = useState<IMasterCardWebhookResponseData>();
    const [searchParams] = useSearchParams();
    const [masterCardWebhookAction, { isLoading }] = useMasterCardWebhookMutation();
    ...
    const checkData = async () => {
        ...
        if (resultIndicator && sessionVersion) {
            try {
                const { response, detail, error, message }: IMasterCardWebhookResponse = await masterCardWebhookAction({
                    resultIndicator,
                    sessionVersion,
                }).unwrap();
                        ...
                        {success ? (
                            ...
                            <div className="w-full flex flex-col gap-y-6">
                                {Array.isArray(message) ? (
                                    ...
                                    <h1 className="text-black text-3xl font-bold leading-10 text-center">
                                        Can't find Mastercard Transaction
                                    </h1>
                                )}
